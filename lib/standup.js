(function() {
  ...
  function markSpoken(pid) {
    ...
    if (typeof refreshTileOverlays === 'function') refreshTileOverlays();
  }
  ...
  function tickSpeakers() {
    ...
    for (var keys = state.speakerTimers.keys(), k = keys.next(); !k.done; k = keys.next()) {
      if (!speaking[k.value]) state.speakerTimers.delete(k.value);
    }
  }
  ...
  function startTimerLoop() {
    ...
    if (!slot) return;
    if (!_timerNode) {
      _timerNode = document.createTextNode('');
      slot.appendChild(_timerNode);
    }
    function tick() {
      var show = false;
      if (state.speaker) {
        if (!state.settings.timerForMeOnly) {
          show = true;
        } else if (state.selfPid && state.speaker.pid === state.selfPid) {
          show = true;
        }
      }
      _timerNode.data = show ? formatElapsed(performance.now() - state.speaker.since) : '';
  ...
  function applyTileDecoration(tile, spoken) {
    tile.style.outline = spoken ? '2px solid #4caf50' : '2px solid #f44336';
  ...
  function clearTileDecoration(tile) {
    tile.style.outline = '';
  ...
  function refreshTileOverlays() {
    ...
    state.participants.forEach(function (entry) {
      ...
      applyTileDecoration(entry._tile, !!entry.spoken);
    });
    remaining.forEach(function (tile) {
      clearTileDecoration(tile);
    });
  }
  ...
  function renderSettings() {
    ...
    fields.forEach(function(f) {
      ...
      if (f.key === 'timerForMeOnly') {
        ...
        help.textContent = 'Non-self timers are a manager/observer use case';
  ...
  function teardown() {
    ...
    if (typeof stopTimerLoop === 'function') stopTimerLoop();
    if (state.decoratedTiles) {
      state.decoratedTiles.forEach(function (tile) {
        tile.style.outline = '';
        tile.style.outlineOffset = '';
        var badge = tile.querySelector('[data-st-badge]');
        if (badge) badge.remove();
