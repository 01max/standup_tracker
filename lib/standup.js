(function() {
  ...
  function tickSpeakers() {
    ...
    document.querySelectorAll('[data-participant-id]').forEach(function (el) {
      ...
      var entry = state.participants.get(pid);
      if (!entry) return;
      var hasSpeakingClass = !!el.querySelector('.sxlEM, .BlxGDf');
      if (!hasSpeakingClass) return;
      speaking[pid] = true;

      var t = state.speakerTimers.get(pid);
      if (!t) {
        t = { startedAt: now, frame: null };
        state.speakerTimers.set(pid, t);
      }
      if (entry.spoken) return;
      if (now - t.startedAt < minTalkMs) return;
      markSpoken(pid);
      state.speaker = { pid: pid, name: entry.name, since: now };
    });

    state.speakerTimers.forEach(function (_, pid) {
      if (!speaking[pid]) state.speakerTimers.delete(pid);
    });
  }
